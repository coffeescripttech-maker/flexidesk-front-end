...
import PhotosGridEdit from "./components/PhotosGridEdit";
...
export default function OwnerListingEdit() {
  ...
  function extractForm(x = {}) {
    return {
      ...
      // demographic fields
      idealFor: Array.isArray(x.idealFor) ? x.idealFor : [],
      workStyle: Array.isArray(x.workStyle) ? x.workStyle : [],
      industries: Array.isArray(x.industries) ? x.industries : [],
  ...
  const save = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        // demographic fields
        idealFor: Array.isArray(form.idealFor) ? form.idealFor : [],
        workStyle: Array.isArray(form.workStyle) ? form.workStyle : [],
    ...
    <OwnerShell headerProps={headerProps} sidebarProps={sidebarProps}>
      ...
      {item && (
        <div className="mt-6 grid gap-4 md:grid-cols-3">
          ...
          <Card title="Work Environment & Demographics" className="md:col-span-2">
            <p className="text-xs text-slate mb-4">
              Help clients find your space by specifying who it's ideal for and what work styles it supports.
              These details appear in search filters and on your listing page.
            </p>
            <div className="grid grid-cols-1 gap-6">
              {/* Ideal For */}
              <div>
                <label className="block text-sm font-medium text-ink mb-2">
                  Ideal For (select all that apply)
                </label>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                  {[
                    ...
                    <label key={value} className="inline-flex items-center gap-2 text-sm">
                      <input
                        type="checkbox"
                        checked={form.idealFor.includes(value)}
                        onChange={(e) => {
                          if (e.target.checked) {
                            setField('idealFor', [...form.idealFor, value]);
                          } else {
                            setField('idealFor', form.idealFor.filter(v => v !== value));
                          }
                        }}
                      />
                      {label}
              ...
              <div>
                <label className="block text-sm font-medium text-ink mb-2">
                  Work Styles Supported (select all that apply)
                </label>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                  {[
                    { value: 'focused', label: 'Focused/Quiet Work' },
                    ...
                    <label key={value} className="inline-flex items-center gap-2 text-sm">
                      <input
                        type="checkbox"
                        checked={form.workStyle.includes(value)}
                        onChange={(e) => {
                          if (e.target.checked) {
                            setField('workStyle', [...form.workStyle, value]);
                          } else {
                            setField('workStyle', form.workStyle.filter(v => v !== value));
                          }
                        }}
                      />
                      {label}
              ...
              <div>
                <label className="block text-sm font-medium text-ink mb-2">
                  Industries Served (optional, select all that apply)
                </label>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                  {[
                    ...
                    <label key={value} className="inline-flex items-center gap-2 text-sm">
                      <input
                        type="checkbox"
                        checked={form.industries.includes(value)}
                        onChange={(e) => {
                          if (e.target.checked) {
                            setField('industries', [...form.industries, value]);
                          } else {
                            setField('industries', form.industries.filter(v => v !== value));
                          }
                        }}
                      />
                      {label}
          ...
          {/* Cancellation Policy */}
          <Card title="Cancellation Policy" className="md:col-span-2">
            <CancellationPolicySection
              policy={form.cancellationPolicy}
              onChange={(policy) => {
                setField("cancellationPolicy", policy);
              }}
            />
          </Card>

          {/* Photos */}
          <Card title="Photos" className="md:col-span-2">
            <PhotosGridEdit 
              item={item} 
              onUpdate={(updates) => setItem((prev) => ({ ...prev, ...updates }))} 
            />
          </Card>
        </div>
      )}
